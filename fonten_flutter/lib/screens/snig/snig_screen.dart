...
class _SnigScreenState extends State<SnigScreen> {
  final TextEditingController _searchController =
      TextEditingController(); // <!> Esto no se si va aca.
  ...
  Widget build(BuildContext context) {
    ...
    if (handler.errorMessage != null) {
      // Si hay un error
      WidgetsBinding.instance.addPostFrameCallback((_) {
        // Asegura que el snackbar se muestre despues de que el arbol de widgets se haya construido
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          Padding(
            padding:
                const EdgeInsets.all(16.0), // Padding de la barra de búsqueda
            child: TextField(
              // TextField de la barra de búsqueda
              controller:
                  _searchController, // Controlador de la barra de búsqueda
              onChanged:
                  _onSearch, // Funcion que se ejecuta cuando se cambia el texto de la barra de búsqueda
          ...
          if (!handler.isLoading &&
              handler.caravanasFiltradas
                  .isNotEmpty) // <DM!> Si hay caravanas cargadas
              ...
              child: handler.caravanasFiltradas.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        return CaravanaItem(
                            caravana: handler.caravanasFiltradas[
                                index], // Caravana a mostrar
                            onToggle: () => handler.toggleSeleccion(
                                index), // Funcion que se ejecuta cuando se cambia el estado de la caravana
                            onDelete: () => handler.eliminarCaravana(
                                index), // Funcion que se ejecuta cuando se elimina la caravana
                            ...
                            onModify: () async {
                              ...
                              final resultado =
                                ...
                                builder: (context) => EditCaravanaScreen(
                                  caravanaModel: handler.caravanasFiltradas[index],
                                  // Pasamos el método de validación del handler
                                  onValidateEID: (val) => handler
                                      .validarDuplicadoEnEdicion(val, index),
                                ),
                              );
                            }
                          );
  ...
  void _mostrarPanelFiltros(BuildContext context, SnigHandler handler) {
    showModalBottomSheet(
      // Muestra un menu inferior para filtrar las caravanas
      context: context, // Contexto de la pantalla
      shape: const RoundedRectangleBorder(
        // Forma del menu inferior
        borderRadius: BorderRadius.vertical(
            top: Radius.circular(20)), // Bordes redondeados
      ),
      builder: (context) {
        // Construye el menu inferior
        // Usamos Consumer para que el panel se redibuje si cambiamos algo
        return Consumer<SnigHandler>(
          // Consumer para que el panel se redibuje si cambiamos algo
          builder: (context, handler, _) {
            // Construye el menu inferior
            return Padding(
              // Padding del menu inferior
              padding: const EdgeInsets.all(20.0), // Padding del menu inferior
              child: Column(
                // Columna del menu inferior
                mainAxisSize: MainAxisSize.min, // Se ajusta al contenido
                crossAxisAlignment:
                    CrossAxisAlignment.start, // Alineacion al inicio
                children: [
                  // Lista de widgets
                  // Título y Botón de Limpiar
                  Row(
                    // Fila del titulo y boton de limpiar
                    mainAxisAlignment:
                        MainAxisAlignment.spaceBetween, // Alineacion al centro
                    children: [
                      // Lista de widgets
                      const Text("Filtros Avanzados", // Titulo
                          style: TextStyle(
                              fontSize: 18,
                              fontWeight:
                                  FontWeight.bold)), // Estilo del titulo
                      TextButton(
                        onPressed: () => handler
                            .limpiarFiltrosAvanzados(), // Funcion que se ejecuta cuando se limpia el filtro
                        child: const Text("Limpiar todo",
                            style: TextStyle(
                                color: Colors.red)), // Texto del boton
                      )
                    ],
                  ),
                  const Divider(),
                  ...
                  Container(
                    padding: const EdgeInsets.symmetric(
                        horizontal: 12), // Padding del contenedor
                    decoration: BoxDecoration(
                        // Estilo del contenedor
                        border: Border.all(
                            color:
                                Colors.grey.shade300), // Borde del contenedor
                        borderRadius: BorderRadius.circular(
                            8) // Radio del borde del contenedor
                        ),
                    child: DropdownButtonHideUnderline(
                      // Oculta la linea de subrayado del dropdown
                      child: DropdownButton<String>(
                        // Dropdown de guia
                        isExpanded:
                            true, // El dropdown ocupa todo el ancho disponible
                        hint: const Text(
                            "Seleccionar Guía"), // Texto que se muestra cuando no hay nada seleccionado
                        value: handler.filtroGia, // Valor seleccionado
                        items: handler.listaGiasDisponibles.map((String gia) {
                          // Lista de items
                          return DropdownMenuItem<String>(
                            // Item del dropdown
                            value: gia, // Valor del item
                            child: Text(gia), // Texto del item
                          );
                        }).toList(),
                        onChanged: (val) => handler.setFiltroGia(
                            val), // Funcion que se ejecuta cuando se cambia el valor del dropdown
                      ),
                    ),
                  ),
                      ...
                      child: Row(
                        ...
                        children: [
                          Text(
                            ...
                            style: TextStyle(
                                color: handler.filtroFecha == null
                                    ? Colors.grey[600]
                                    : Colors.black),
                          ),
  ...
  Widget _buildSummaryBar(SnigHandler handler) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          _buildStat(
            // Boton Muestra todo los totales de caravanas
          ...
          _buildStat(
            // Boton filtra las caravanas que estan en simulador
          ...
          _buildStat(
            // Boton filtra las caravanas que no estan en simulador
  ...
  Widget _buildStat(
    ...
    return InkWell(
      onTap: () => handler.setFilter(
          type), // Funcion que se ejecuta cuando se cambia el estado de la caravana
  ...
  Widget _buildTripleActionBar(SnigHandler handler) {
    ...
    return AnimatedContainer(
      duration: const Duration(
          milliseconds: 300), // Tiempo de la transición (suave para el usuario)
      height: handler.showBottomActions
          ? expandedHeight
          : // Si es true, la algura menu desplegado
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          ...
          BoxShadow(
              color:
                  Colors.black12, // Sombra sutil para dar efecto de elevación
      ...
      child: Column(
        ...
        children: [
          ...
          GestureDetector(
            onTap: () => handler
                .toggleBottomActions(), // Cambia el estado de visibilidad
            child: Container(
              ...
              child: Icon(
                handler.showBottomActions
                    ...
                    : // Flecha hacia abajo si está expandido
                    Icons
                        .keyboard_arrow_up, // Flecha hacia arriba si está colapsado
                  ...
                  _buildBottomButton(
                    ...
                    onTap: () async {
                      ...
                      await Navigator.push(
                        // Detiene la ejecución aquí (await) hasta que la pantalla nueva se cierre (pop).
                        context, // Necesario para saber dónde estamos en el árbol de widgets.
                        MaterialPageRoute(
                          // Crea la animación estándar de transición de pantalla (deslizamiento).
                          builder: (context) => ChangeNotifierProvider.value(
                            // Inyecta un Provider existente en la nueva ruta.
                            ...
                            value: Provider.of<SnigHandler>(context,
                                listen: false),
                      ...
                      if (context.mounted) {
                        Provider.of<SnigHandler>(context, listen: false)
                            .refrescarDesdeService();
                      }
                    },
                  ),
                ],
